 ...
 */
export function mergeProjects( projects: Project[], info: { title: string, description: string, version: string, baseUrl: string } ): Project {
  // setup merged project
  let mergedProject: Project = {
    info: new ProjectInfo( info.title, undefined, info.version, [ info.version ], [], info.description )
  };
  if(info.baseUrl){
